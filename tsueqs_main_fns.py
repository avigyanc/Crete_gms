...
# -*- coding: utf-8 -*-
"""
Created on Tue Feb 23 10:16:01 2021
...
##############################################################################
def vel_to_acc(datastream,fmin,fmax,order,zerophase=True):
    ...
    datastream[0].detrend('linear')
    datastream[0].taper(max_percentage=0.05)
    datastream[0].data-=np.mean(datastream[0].data)
    vel_amplitude = datastream[0].data
    
    ## And times:
    vel_times = datastream[0].times()
    
    ## INtegrate the acceration time series to get velocity:
    acc_amplitude = np.diff(vel_amplitude)
    
    ## Make a copy of the old stream object:
    acc_timeseries_stream = datastream.copy()
    
    ## Put the integrated velocity in there in place of accel:
    acc_timeseries_stream[0].data = acc_amplitude
    #acc_timeseries_stream.filter('bandpass', freqmin=.001,freqmax = 1, corners=2, zerophase=True)
    fsample = 1/acc_timeseries_stream[0].stats.delta
    fnyquist=(fsample)/2
    b, a = butter(4, array(1/15.)/(fnyquist),'highpass')
    if zerophase==True:
        data_filt=filtfilt(b,a,acc_timeseries_stream[0].data)
    else:
        data_filt=lfilter(b,a,acc_timeseries_stream[0].data)
